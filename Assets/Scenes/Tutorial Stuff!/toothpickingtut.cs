...
public class toothpickingtut : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            Popup.MakePopup("Drag and drop the sandwhich to the man");
    ...
    void FixedUpdate()
    {
        ...
        if (canorderup)
        {
            if(Physics.Raycast(transform.position, Vector3.forward, out hit, 100, LayerMask.GetMask(LayerMask.LayerToName(3))))
            {
                ...
                guyTouching.GetComponent<GuyTut>().DisplayText("This sandwhich is terrible! And you took too long!");
                Popup.MakePopup("Hop the table with SPACEBAR and beat his ass.");
            }
